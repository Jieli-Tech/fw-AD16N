...
typedef struct _Reg_Message_Start_ {
    void *priv;
    u32(*MessageStart)();
} Reg_Message_Start;

typedef struct _Reg_Message_End_ {
    void *priv;
    u32(*MessageEnd)();
} Reg_Message_End;
...
enum {
    ...
    CMD_MIDI_CTRL_GOON,					//midi 琴消耗事件
    CMD_MIDI_REG_MESSAGE_START,			// 配置midi琴注册事件保护
    CMD_MIDI_REG_MESSAGE_END			// 配置midi琴注册完事件关闭保护
};
