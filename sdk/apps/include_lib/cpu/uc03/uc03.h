...
typedef struct {
    ...
    __RW __u16(DAC_SWN);
    __RW __u32(DAC_VL0);
    __RO __u32(RESERVED_AUD0); //(DAC_VL1);
    __RW __u32(DAC_TM0);
    __RW __u32(RESERVED_AUD1); //(DAC_TM1);
    __RW __u32(RESERVED_AUD2); //(DAC_DB0);
    __RW __u32(RESERVED_AUD3); //(DAC_DB1);
    __WO __u16(DAC_COP);
    __RW __u32(ADC_CON);
    __RW __u32(ADC_ADR);
    __RW __u16(ADC_LEN);
    ...
    __RW __u16(ADC_SRP);
    __RW __u16(ADC_SRN);
    __RW __u32(RESERVED_AUD4); //(MBG_CON);
    __RW __u32(AUD_CON);
    __RW __u16(ADC_CON1);
    __RW __u32(DCC_CON0);
    __RW __u32(DCC_CON1);
...
#define JL_TIMER0_BASE                  (ls_base + map_adr(0x05, 0x00))
#define JL_TIMER0                       ((JL_TIMER_TypeDef     *)JL_TIMER0_BASE)

#define JL_TIMER1_BASE                  (ls_base + map_adr(0x06, 0x00))
#define JL_TIMER1                       ((JL_TIMER_TypeDef     *)JL_TIMER1_BASE)

#define JL_TIMER2_BASE                  (ls_base + map_adr(0x07, 0x00))
