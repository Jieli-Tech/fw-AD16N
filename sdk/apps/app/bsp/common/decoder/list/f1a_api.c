...
#pragma bss_seg(".f1a_data")
#pragma data_seg(".f1a_data")
#pragma const_seg(".f1a_api.text.const")
#pragma code_seg(".f1a_api.text")
#pragma str_literal_override(".f1a_api.text.const")
...
#define LOG_TAG             "[f1a_api]"
#include "log.h"

#define F1A_OBUF_SIZE   (DAC_DECODER_BUF_SIZE * 2)
/* #define F1A_DBUF_SIZE   (0xc70) */
#define F1A_OUTPUT_MAX_SIZE (32 * 2)
#define F1A_KICK_SIZE   (F1A_OBUF_SIZE - (F1A_OUTPUT_MAX_SIZE * 2))
dec_obj dec_f1a_hld[MAX_F1A_CHANNEL] AT(.f1a.data.bss);
...
struct if_decoder_io f1a_dec_io[MAX_F1A_CHANNEL] AT(.f1a_1_buf);
...
u32 f1a_decode_index(void *strm, u32 index, dec_obj **p_dec, void *p_dp_buf, f1x_data *p_f1x_data)
{
    ...
    memcpy((void *)&f1a_dec_io[index], p_strm->io, sizeof(struct if_decoder_io));
    f1a_dec_io[index].priv      = &dec_f1a_hld[index];
    ...
    if (!(B_DEC_NO_CHECK & p_strm->strm_ctl)) {
        if (check_ext_api(g_file_sname, f1a_ext, 4)) {
            ...
            ctype = 'b';
            /* ops->get_dec_inf(p_dbuf)->sr = 32000; */
            /* i = 32000; */
            /* ops->dec_confing(p_dbuf, CMD_SET_SAMPLE, &i); */
            ...
            ctype = 'x';
            void *pf1x = f1x_play_api(p_dec_hld->p_file, (u8 *)g_loop_tab, sizeof(g_loop_tab), index, t_par);
            if (NULL == pf1x) {
                log_error("f1x null\n");
                return E_F1X_FORMAT;
            }
...
//--------------------------------
u32 f1a_decode_api_1(void *strm, void **p_pdec, void *t_dp_buf)
{
    return f1a_decode_index(strm, 0, (dec_obj **)p_pdec, t_dp_buf, &g_f1x1_data);
}

u32 f1a_decode_api_2(void *strm, void **p_pdec, void *t_dp_buf)
{
#if (MAX_F1A_CHANNEL > 1)
    return f1a_decode_index(strm, 1, (dec_obj **)p_pdec, t_dp_buf, &g_f1x2_data);
#else
    return E_F1A_OUTRGE;
#endif
}
